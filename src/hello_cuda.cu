...
//Petit kernel (jamais lance si aucun device n'est dispo)
__global__ void addOne(int* v) { *v += 1; }

int main() {
    //1) Detection du runtime/driver + devices
    ...
    if (e != cudaSuccess) {
        ...
        return 0; //OK pour notre projet: on continue coté CPU
    }
    if (count == 0) {
    ...
    if (e != cudaSuccess) {
        ...
        return 0; //on s'arrete proprement, le projet reste fonctionnel coté CPU
    }
