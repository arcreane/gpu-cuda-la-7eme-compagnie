...
* La portabilité CPU, et donc fallback automatique si pas de GPU
* La compatibilité immédiate Raylib + Qt
* La réutilisation future (simulation autonome)

Les données manipulées sont définies dans particles_types.hpp. Elles sont communes à Raylib, QT, au backend CPU et GPU.

La structure Particle par exemple définit la position, la vitesse et la couleur de chaque particule.

Afin de séparer le moteur de calcul du rendu, nous utilisons une interface commune. 
Deux implémentations existent : compute_cpu.cpp, pour la mise à jour séquentielle des particules, et compute_cuda.cu pour l'exécution parallèle sur GPU.
